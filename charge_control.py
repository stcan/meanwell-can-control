...
ChargeVoltage = 2760
DischargeVoltage = 2560
MaxChargeCurrent = 2500
MaxDischargeCurrent = 2500

# Init CAN Bus
p = subprocess.run(["./bic22.py" , "can_up"])

# Write Charge / Discharge Voltages
p = subprocess.run(["./bic22.py", "cvset", str(ChargeVoltage)])
p = subprocess.run(["./bic22.py", "dvset", str(DischargeVoltage)])
...
def control_power():
    ...
    v  = subprocess.run(["./bic22.py", "vread"], capture_output=True, text=True)
    v_now = float(v.stdout)
    a  = subprocess.run(["./bic22.py", "cread"], capture_output=True, text=True)
    ...
    if Current > 0:
        ...
        p = subprocess.run(["./bic22.py" , "charge"])
        c = subprocess.run(["./bic22.py" , "ccset" ,  str(IntCurrent)]) 
    ...
    if Current < -10: 
        ...
        p = subprocess.run(["./bic22.py" , "discharge"])
        c = subprocess.run(["./bic22.py" , "dcset", str(IntCurrent)]) 
...
schedule.every(4).seconds.do(control_power)      # Aufruf der Regelroutine alle 5s
